...
Targeted bug-fix release for the Ember Circuit intake worksheets (8 worksheets, 192 interaction slots). All 13 commits since 0.0.35 are fixes — no new features, no schema changes, no grammar additions. The Ember Circuit intake workflow now reaches "Preview new packet" → "Create previewed packet" end-to-end with the custom-drawn checkbox visual, the electric-circuit color chain, and the project-local `_responses/` folder enforced end-to-end.

- **Identity fix (`a97f615`)** — the canonical hub install now reports the public plugin identity `hearth-and-code-governance-lab` (author Scott Rallya, homepage `https://hearthandcode.dev`) when the dev override is not applied. Earlier scratch-vault builds used the prototype identity `hcc-widget-lab`; the public source with the dev override no longer leaks the prototype identity into a production install.
- **YAML import/export round-trip (`28a242f`, `29f6cea`)** — the packet panel now exports the in-memory draft as YAML (`Export draft as YAML`) and imports a previously-exported YAML back into the session (`Import draft from YAML`). Both `record_type: hcc-worksheet-session-draft` (mutable draft) and `record_type: hcc-worksheet-response-packet` (immutable packet) shapes are accepted on import; the controller validates the worksheet id, the interaction ids, and the in-memory non-empty state with a `discard: true` opt-in. Failures are bound to `HCC-IMPORT-PARSE/SCHEMA/WORKSHEET/SCOPE/NONDISCARD` codes so the gate path is auditable.
- **Custom-drawn checkbox visual (`84f3f09`, `7e2c7aa`, `5ffeadc`, `6ce6dc2`, `8b09a52`)** — the option-list, packet-confirm, and packet-import-discard inputs now use the matrix-choice pattern: `appearance: none` (plus `-webkit-appearance: none` for Chromium Electron builds), a real border, `display: inline-block` + `position: relative` (sidestepping the `display: grid` blockification on a flex child), and a `::before` pseudo-element for the checkmark scaled between `scale(0)` and `scale(1)` on `:checked`. The visual is persistent regardless of mouse position. The unchecked border uses `--hcc-text-muted` for WCAG 1.4.11 3:1 contrast against `--hcc-surface` in the ember-circuit theme; the checked state uses the electric-circuit color chain (`--hcc-signal-violet` → `--hcc-signal-cyan` → `--hcc-accent-bright`) for border, inset ring, outer glow, and checkmark.
- **Matrix-choice lavender alignment (`541f4e3`)** — the matrix-choice `:checked` ring now uses the same electric-circuit color chain as the option-list and packet-confirm inputs; the earlier `--hcc-accent` (orange) is removed, so the forge aesthetic is consistent across every checkbox and radio in the workbook.
- **Ranking layout (`6ce6dc2`)** — `column-gap: 0.75rem` on the ranking item and a `.hcc-widget__ranking-item__buttons` wrapper so the up/down button pair is grouped with a consistent separator from the option label.
- **Forced-colors override (`8b09a52`)** — the `@media (forced-colors: active)` block from the matrix-choice input is extended to the option-list and packet-confirm/import-discard inputs so Windows High Contrast mode renders a native appearance with system-color borders and a Unicode `✓` instead of the clip-path `::before`.
- **Mark Answers Finalized workflow (`f8e32ee`, `04c34c3`)** — after `Execute Import`, the panel auto-sets `reviewedFingerprint` to the current release fingerprint so the user can immediately click `Mark answers finalized` without first clicking `Review worksheet`. The finalize step still requires the explicit click because `finalizedFingerprint` stays null until the user clicks the button.
- **Adapter folder threading (`f31e41e`)** — the response-packet adapter factory (`createResponsePacketAdapter`) now accepts an optional folder argument, and the controller's `adapter` option is a factory that takes the worksheet's resolved folder. Every `createOnly` / `readExplicit` call lands on an adapter constructed for the worksheet's `target_folder_override`, so the immutable packet lands in the project-local `_responses/` folder instead of being rejected with `HCC-VAULT-TARGET`. The legacy `Intake/HCC Responses/` literal remains the default for one release for backward compatibility.
- **Visible `:checked` ring (`f50bdef`)** — the option-list radios and checkboxes get an inset 2px ring + outer 4px glow in the electric-circuit color chain on `:checked`. The packet-confirm and packet-import-discard inputs receive the same treatment via the shared `:checked` rule.
- **Structural restore (`6ce6dc2`)** — a missing closing `}` in the prior commit's `:checked` input rule that was absorbing the candidate dashed-border, the `.hcc-widget__input` width, and the `.hcc-widget__ranking-item` grid layout is restored. Brace balance is 321/321 after the restore.
- **Version bumped** to `0.0.36` across `package.json`, `manifest.json`, and `versions.json`; Obsidian minimum version remains `1.13.4`.

Verified locally: `npm run check` clean; `npm run test` 47 files, 330 tests, all PASS; `npm run proof` exit 0; one real Ember Circuit packet landed at `04-workspace--scriptorium/projects/ember-circuit-brand-system/intake/_responses/ember-circuit-overview--04-workspace--scriptorium-projects-ember-circuit--session-20260817184605219.yaml` with exact digest `sha256:fb573514a22941cc1c2034aa42d4cbc5af6fe800c10d336158c3207a0d720b36`.
