...
The optional settings panel stores one versioned plugin-owned `data.json` file in Obsidian's plugin configuration directory only after the user changes a preference. It contains exactly four presentation preferences: default presentation, explanatory-detail level, interaction density, and routine-notice level. Loading the plugin with no settings file does not create one. The file contains no responses, note contents, paths, packet locators, vault name, account data, credentials, governance decisions, or telemetry. Obsidian Sync or another vault synchronization tool may copy plugin configuration according to that tool's own configuration and boundary.
